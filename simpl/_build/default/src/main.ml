...
and typeof_bop ctx bop e1 e2 =
  ...
  | Addf, TFloat, TFloat -> TFloat
  | Multf, TFloat, TFloat -> TFloat
  | Powerf, TFloat, TFloat -> TFloat
  | _ -> failwith bop_err
...
let rec step : expr -> expr = function
  | Int _ | Bool _ | Float _ -> failwith "Does not step"
...
and step_bop bop e1 e2 = match bop, e1, e2 with
  ...
  | Power, Int a, Int b -> Int (int_of_float ((float_of_int a) ** (float_of_int b)))
  | Addf, Float a, Float b -> Float (a +. b)
  | Multf, Float a, Float b -> Float (a *. b)
  | Powerf, Float a, Float b -> Float (a ** b)
  | _ -> failwith bop_err
